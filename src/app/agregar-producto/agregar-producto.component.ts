...
export class AgregarProductoComponent implements OnInit {
  ...
  async guardar() {
    if (!this.productoModel.nombre) {
      return alert("Escriba un nombre del servicio o trabajo*");
    }
    if (!this.productoModel.descripcion) {
      return alert("Describa el servicio o trabajo*");
    }
    if (!this.productoModel.precio) {
      return alert("Escriba un precio*");
    }
    ...
    this.snackBar.open("Servicio guardado", "", {
      duration: 1500,
